...
from AaSystem.AaSystemControl.InstallationInformation import InstallationShellScript
...
class RemoteSSH(ICommand):
    def Execute(self):
        ...
        ConnectAndPerformCommands(hostname, port, username, password, commands)
...
class MigrateAndPerformCommands(ICommand):
    def Execute(self):
        if self.CheckHelpRequested():
            return
        if self.CheckMinimunRequiredParameters(4):
        ...
        password = self.request[3]
        commands = None if len(self.request) <= 4 else self.request[4:]
        allCommands = [InstallationShellScript]
        allCommands.extend(commands)
        ConnectAndPerformCommands(hostname, port, username, password, allCommands)

    def HelpRequested(self):
        PrintAndLog("\n"
                    "       Connects a remote machine via an ssh connection, installs the AttackAgent and performs "
                    "commands\n"
                    "       Command parameters: [hostname] [port] [username] [password] [command] [command] [command]\n"
                    "                     'hostname' - The address of the machine you are attempting to connect to\n"
                    "                     'port' - The port the connection will be made on\n"
...
MigrateAndPerformCommands.PublicFacing = "migrate"
